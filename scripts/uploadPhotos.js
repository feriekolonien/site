...
async function runScript() {
  ...
  console.log(
    `Will upload ${entries.length} files in ${paginated.length} batches`,
  );
  for (let index = 0; index < paginated.length; index++) {
    console.log(`starting batch #${index} (${paginated[index].length} files)`);
    const promises = paginated[index].map(file => uploadImageAsset(file)); // returns promise
